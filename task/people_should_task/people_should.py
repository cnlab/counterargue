    ...
    for idx,cur_item in enumerate(block_items):
        ...
        #for frame in range(trial_time):
            ...
            if cur_cond[:-4]=='AGREE':
                ...
                resp3.draw()
            if not key:
                key = event.getKeys(timeStamped=globalClock, keyList=('b','y'))
            else:
                if key[0][0]=='b':
                    if cur_cond[:-4]=='AGREE':
                        resp1.setColor('#FF0000')
                        r1_clear.append(frame+30)
                    elif cur_cond.startswith('INFAVOR') or cur_cond.startswith('AGAINST'):
                    ...
                    elif cur_cond.startswith('AGAINST') or cur_cond.startswith('INFAVOR') :
                        resp3.setColor('#FF0000')
                        r3_clear.append(frame+30)
                    resp.append((key[0][0],key[0][1],cur_item))
                    key=None
